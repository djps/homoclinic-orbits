...
C   pp.f
C
C	parameter-plane continuation : finding the spectrum of the 
C floquet multipliers as an algeraic continuation problem by tracing the 
C from the bifurcation curves starting from the classical value m=2.
C
C 	continuation is performed by changing lambda and then seeing 
C the changes in m.
C
C---------------------------------------------------------------------- 
C---------------------------------------------------------------------- 
...
C
      ...
      DIMENSION wr(n),wi(n),wrdum(n),widum(n)
...
C
	do k = 1,n
     ...
     &IWORK(18),IWORK(19),IWORK(20), xlambda
	    return
	  else
	  end if
	  do i = 1,n
...
C
C Function to continue
C
	F(1) = wi(6)**2.d0 + wr(6)**2.d0- 1.d0 
C
      RETURN 
      END 
C
