C---------------------------------------------------------------------- 
C---------------------------------------------------------------------- 
C   mag : Nondimensional Formulation of a Cosserat rod under 
C         tension, torque and body force from a given homoclinic 
C         solution in the dynamic case.
C
C         This draft uses projection boundary conditions.
C
C---------------------------------------------------------------------- 
C---------------------------------------------------------------------- 
C 
...
C
	...
	xD        = PAR(9)
C
	xepsilon  = xepsilon0 + xalpha*(datan(xbeta*xlambda))**2.d0
C
	F(1)  = (1.d0+xnu)*F2*XM3 - F3*XM2 + 
     & xlambda*(1.d0+xepsilon*F3)*2.d0*(Q1*Q4 + Q2*Q3) 

	F(2)  = (1.d0+xrho)*F3*XM1 - (1.d0+xnu)*F1*XM3 -
     & xlambda*(1.d0+xepsilon*F3)*2.d0*(Q1*Q3 - Q2*Q4) 
...
C
C Find full length end-deflections
C
	pi        =  4.d0*datan(1.d0)
C
	xnu       =  0.333333333333E+00
C
	xrho      =  0.000000000000E+00
	xm        =  1.9d0
	xlambda   =  0.05d0
	xepsilon0 =  0.d0
	xT        =  0.749561843331E+02
	xalpha    =  3.d0/(5.d0*pi**2)
	xbeta     =  20.d0*dtan(pi/dsqrt(6.d0))
C
	xR        =  0.499972803020E+00
	xD        =  0.139010842793E+01
C
	PAR(1)    = xrho
	PAR(2)    = xm
	PAR(3)    = xlambda
...
C
	...
	PAR(9)    = 2.d0*PAR(9)
C
! 	xepsilon  = xepsilon0 + xalpha*(datan(xbeta*xlambda))**2d0
! 	short  = ( (1.d0+xepsilon)*xT - 0.8106169434E+02 )
! 	print*, xepsilon, short, xD - short
C
      RETURN
      END
C            
...
C
      ...
      PARAMETER (liwork=nrdens+20, lrpar=9, lipar=9)
      DIMENSION rpar(lrpar),ipar(lipar),work(lwork),iwork(liwork),y(n)
C
      DIMENSION vr(n,n),vi(n,n),vidum(n,n),vrdum(n,n)
      DIMENSION wr(n),wi(n),wrdum(n),widum(n),xnorm(n)
      DIMENSION z(n,n),fv1(n)
      DIMENSION stationary(10),v1(10),v2(10),v(6,10)
      DIMENSION xMonodromy(n,n), xInitial(n,n)
      INTEGER ierr,iv1(n),iR
      EXTERNAL USRFUN,SOLOUT
C
	pi        = 4.d0*datan(1.d0)
C
	xnu       = 1.d0/3.d0
...
C
	xepsilon  = xepsilon0 + xalpha*(datan(xbeta*xlambda))**2.d0
C
! 	print*, xepsilon, xepsilon0, xalpha, xbeta, xT, xD, PAR(6)
C
	x      = 0.d0
...
C
	do k = 1,6
	  ...
	  call dop853(n,usrfun,x,y,xend,xtolx,xtolf,itol,solout,iout,
     &work,lwork,iwork,liwork,rpar,ipar,idid)
	  if (idid.ne.1) then
	    print*, i, k, " argh ", idid, IWORK(17),
     &IWORK(18),IWORK(19),IWORK(20), xlambda, xT, xR, xD
	    return
	  else
	  end if
	  do i = 1,n
...
C 
C Take transpose of monodromy matrix
C
	do i = 1,n
		do j = 1,n
			vrdum(j,i) = xMonodromy(i,j)
		end do
	end do
	do i = 1,n
		do j = 1,n
			xMonodromy(i,j) = vrdum(i,j)
		end do
	end do
	do i =1,n
		do j= 1,n
...
C
C Order the Floquet exponents and multipliers according to the size of the magnitude of the exponents
C
	do i = 1,n-1
	  do j = i+1,n
	    if ( (wi(i)**2.+wr(i)**2.).gt.(wi(j)**2.+wr(j)**2.) ) then
			wrdum(i) = wr(i)
			widum(i) = wi(i)
			wr(i)    = wr(j)
			wi(i)    = wi(j)
...
C
	do i = 1,n-1
		jflag = 0
		do j = 1,n
			if ( dabs(vr(j,i)-vr(j,i+1)).lt.zero ) then
				jflag = jflag + 1
				if ( jflag.eq.n ) then 
					do k = 1,n
						vr(k,i+1) = vi(k,i)
...
C
C Create projection boundary condition vector
C
	do i = 1,6
	  do j = 1,10
	    v(i,j) = 0.d0
...
C
C Projection boundary conditions - over unstable and centre directions
C
	do i = 1,4
	  do j = 1,10
	    FB(i) = FB(i) + ( U0(j) - stationary(j) )*v(i,j)
	  end do 
	end do
! 
! 	do i = 5,6
! 	  do j = 1,10
! 	    FB(i) = FB(i) + ( U1(j) - stationary(j) )*v(i,j)
! 	  end do 
! 	end do
C
C Symmetric section conditions
C
	if (iR.eq.1) then 
	  FB(5) = U1(1)
	  FB(6) = U1(4)
...
C
C 'Integral' fixing boundary conditions
C
	FB(8)  = U0(3) - 1.d0
	FB(9)  = U0(6) - 1.d0
	FB(10) = U0(9) - 1.d0
	FB(11) = U0(10)
C
C Post-processing conditions, centre line conditions, x, y, z
C
	FB(12) = U0(11)
	FB(13) = U0(12)
	FB(14) = U0(13)
C
C Additional parameters in order to free parameters containing the end displacements: the end-shortening and the end rotation.
C
	short  = 2.d0*( (1.d0+xepsilon)*xT - U1(13) )
	FB(15) = xD - short
! C
...
!  	FB(16) = xR - 2.d0*rotate
C
C Output to check boundary conditions
C
! 	if (NBC.eq.13) then
! 		write(99,'(13e20.12)') (FB(i),i=1,13)
! 	else if (NBC.eq.14) then
! 		write(99,'(14e20.12)') (FB(i),i=1,14)
! 	elseif (NBC.eq.15) then
! 		write(99,'(15e20.12)') (FB(i),i=1,15)
! 	elseif (NBC.eq.16) then
! 		write(99,'(16e20.12)') (FB(i),i=1,16)
! 	else
! 	end if
C
! 	write(99,'(4e20.12)') U0(10), U1(10),
!      &U0(7)**2 + U0(8)**2 + U0(9)**2 + U0(10)**2,
!      &U1(7)**2 + U1(8)**2 + U1(9)**2 + U1(10)**2
C
C
      RETURN
      END      
...
C
      ...
      END 
C
      SUBROUTINE USRFUN(N,X,Y,F,RPAR,IPAR)
C     ---------- ------ 
C      
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      PARAMETER (LRPAR=9, LIPAR=9) 
...
C
	...
	Q2  = Y(6)
C
	Q3_star = dcos( X*(1.d0+xnu)/2.d0 )
	Q4_star = -dsin( X*(1.d0+xnu)/2.d0 )
C
	F(1) = (1.d0+xnu)*F2 - XM2 +
     & 2.d0*xlambda*(1.d0+xepsilon)*(Q4_star*Q1+Q3_star*Q2)
C
	F(2) =-(1.d0+xnu)*F1 + (1.d0+xrho)*XM1 -
     & 2.d0*xlambda*(1.d0+xepsilon)*(Q3_star*Q1-Q4_star*Q2)
C
	F(3) = F2*(1.d0+xepsilon)/(xm**2.d0) + xnu*XM2
C
	F(4) =-F1*(1.d0+xepsilon)/(xm**2.d0) + (xrho-xnu)*XM1
C
	F(5) = (1.d0+xrho)*Q4_star*XM1/2.d0 + (1.d0+xnu)*Q2/2.d0 -
     & Q3_star*XM2/2.d0
C
	F(6) = (1.d0+xrho)*Q3_star*XM1/2.d0 - (1.d0+xnu)*Q1/2.d0 +
     & Q4_star*XM2/2.d0 
C
C
      RETURN
